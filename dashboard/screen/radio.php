...
<body>
    ...
    <div class="cont-geral">
        <div class="cont-secundario-radio">
            ...
            <hr class="separ">
            <div class="sepp-radios">
                <div class="emp-radios">
                    <div class="subradios-emp" id="subradios-emp">
                    <?php
                ...
                <div class="rads">
                    <div class="sub-rads" id="sub-rads">
                ...
                <div class="radsinform">
                    <div class="subradsinform">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const subContCamerasDiv = document.querySelector('.sub-rads');
            ...
            if (subContCamerasDiv) {
                ...
                subContCamerasDiv.addEventListener('click', function (event) {
                    ...
                    if (event.target.classList.contains('info-rads') || event.target.classList.contains('fa-chart-bar')) {
                        event.preventDefault(); // Evita que o link seja seguido

                        // Obtém o elemento pai que contém as informações da câmera
                        const cameraInfoContainer = event.target.closest('.empsview');
                        ...
                        if (cameraInfoContainer) {
                            ...
                            xhr.open('GET', `buscar_informacoes_radio.php?ip=${ip}&nomeRadio=${nomeCamera}&localRadio=${localCamera}&portaRadio=${portaCamera}&usuarioRadio=${usuarioCamera}&pswRadio=${pswCamera}`, true);
                            xhr.onload = function () {
                                if (xhr.status === 200) {
                                    const subContInfoDiv = document.querySelector('.subradsinform');
        ...
        document.addEventListener('DOMContentLoaded', function () {
            console.log("DOMContentLoaded event triggered.");
            const empresasDiv = document.getElementById('subradios-emp');
            ...
            if (empresasDiv) {
                ...
                empresasDiv.addEventListener('click', function (event) {
                    ...
                    const target = event.target.closest('.empview');
                    if (target) {
                        const empresaId = target.getAttribute('data-empresa-id');

                        const xhr = new XMLHttpRequest();
                        xhr.open('GET', `buscar_radio.php?empresa_id=${empresaId}`, true);
                        xhr.onload = function () {
                            if (xhr.status === 200) { 
                                const subContCamerasDiv = document.getElementById('sub-rads');
                                if (subContCamerasDiv) {
                                    subContCamerasDiv.innerHTML = xhr.responseText;
                                } else {
                                    console.error('Element with id "sub-rads" not found.');
                                }
                            } else {
